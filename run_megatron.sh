...
export GLOBAL_BATCH_SIZE_CODEGEEX=$GLOBAL_BATCH_SIZE

export MICRO_BATCH_SIZE_CODEGEEX=$MICRO_BATCH_SIZE
...
elif [ "$MODEL" = "CODEGEEX" ]; then
   export CHECKPOINT_PATH=/home/yuke/lyd/Megatron-LM/checkpoints/codegeex
	export VOCAB_FILE=/home/yuke/lyd/Megatron-LM/model/codegeex-vocab.json
	export MERGE_FILE=/home/yuke/lyd/Megatron-LM/model/codegeex-merges.txt
	export DATA_PATH=/home/yuke/lyd/Megatron-LM/my-codegeex_text_document  
...
if [ "$MODEL" = "gpt2" ]; then
    ...
    /home/ldai8/data/conda3/envs/pytorchNCCL-hao/bin/python -m torch.distributed.launch $DISTRIBUTED_ARGS pretrain_codegeex.py \
	--num-layers 39 --hidden-size 5120 --num-attention-heads 40 --max-position-embeddings 2048 \
	--micro-batch-size $MICRO_BATCH_SIZE_CODEGEEX --global-batch-size $GLOBAL_BATCH_SIZE_CODEGEEX \
	--lr-decay-style cosine \
	--attention-softmax-in-fp32 --load "$CHECKPOINT_PATH" --layernorm-epsilon 1e-5 --fp16 \
	--ws-encoding-start-id 10 \
	--ws-encoding-length 10 --make-vocab-size-divisible-by 52224 --seq-length 2048 \
	--data-path $DATA_PATH > /home/ldai8/bash/Megatron_data_output_profile/${MODEL}/2gpus-mbs${MICRO_BATCH_SIZE_CODEGEEX}/${PROTOCOL}/${MODEL}-${PROTOCOL}-2gpus-${NODE_RANK}-mbs-${MICRO_BATCH_SIZE_CODEGEEX}.out
else
